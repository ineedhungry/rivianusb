...
function nm_add_ap () {
  ...
  nmcli con delete RIVIANUSB_AP &> /dev/null || true
  nmcli con add type wifi ifname ap0 mode ap con-name RIVIANUSB_AP ssid "$AP_SSID" || return 1
  # don't set band and channel, because that is controlled by the $WLAN interface
  #nmcli con modify RIVIANUSB_AP 802-11-wireless.band bg
  #nmcli con modify RIVIANUSB_AP 802-11-wireless.channel 6
  nmcli con modify RIVIANUSB_AP 802-11-wireless-security.key-mgmt wpa-psk || return 1
  nmcli con modify RIVIANUSB_AP 802-11-wireless-security.psk "$AP_PASS" || return 1
  IP=${AP_IP:-"192.168.66.1"}
  nmcli con modify RIVIANUSB_AP ipv4.addr "$IP/24" || return 1
  nmcli con modify RIVIANUSB_AP ipv4.method shared || return 1
  nmcli con modify RIVIANUSB_AP ipv6.method disabled || return 1
  cat > /etc/network/if-up.d/rivianusb-ap << EOF
#!/bin/bash
...
then
  ...
  nmcli con up RIVIANUSB_AP
fi

EOF
  chmod a+x /etc/network/if-up.d/rivianusb-ap || return 1
}
