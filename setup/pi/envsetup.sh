...
function read_setup_variables {
  if [ -z "${setup_file+x}" ]
  then
    local -r setup_file=/root/rivianusb_setup_variables.conf
  fi
  if [ -e $setup_file ]
  then
  ...
  newnamefor[rivian_email]=rivian_EMAIL
  newnamefor[rivian_password]=rivian_PASSWORD
  newnamefor[rivian_vin]=rivian_VIN
  ...
  export rivianUSB_HOSTNAME=${rivianUSB_HOSTNAME:-rivianusb}
  export NOTIFICATION_TITLE=${NOTIFICATION_TITLE:-${rivianUSB_HOSTNAME}}
...
then
  ...
  complete -W "diagnose upgrade install" setup-rivianusb
fi
...
if [ -f /rivianusb/cmdline.txt ]
then
  export CMDLINE_PATH=/rivianusb/cmdline.txt
else
  export CMDLINE_PATH=/dev/null
fi

if [ -f /rivianusb/config.txt ]
then
  export PICONFIG_PATH=/rivianusb/config.txt
else
