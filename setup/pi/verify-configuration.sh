...
function check_supported_hardware () {
  ...
  setup_progress "(only Pi Zero W, Pi 4, and Pi 5 have the necessary hardware to run rivianusb)"
  exit 1
}
...
function check_setup_rivianusb () {
  if [ ! -e /root/bin/setup-rivianusb ]
  then
    setup_progress "STOP: setup-rivianusb is not in /root/bin"
    exit 1
  fi
  ...
  then
    setup_progress "STOP: $0 must be called from setup-rivianusb: $parent"
    exit 1
  fi
}
